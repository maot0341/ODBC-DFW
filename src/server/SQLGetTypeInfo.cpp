...
///---------------------------------------------------------------------------
bool
CSQLGetTypeInfo::read (ULONG iRow, ULONG nRow, idl::typRecord & raRecord) const
{
//		return false;
	...
	for (i=0; i<nRow; i++)
	{
		if (i >= nRows)
			break;
		short iType = m_aKeys[i];
		std::map<short,CSQLTypeInfo*>::const_iterator iInfo = m_aData.find(iType);
		assert (iInfo != m_aData.end());
		const CSQLTypeInfo * pInfo = iInfo->second;
		assert (pInfo);
		j = i * nCols;
		idlcpy (raRecord[j +  0], pInfo->TYPE_NAME());
		idlcpy (raRecord[j +  1], pInfo->DATA_TYPE());
		idlcpy (raRecord[j +  2], pInfo->COLUMN_SIZE());
		idlcpy (raRecord[j +  3], pInfo->LITERAL_PREFIX());
		idlcpy (raRecord[j +  4], pInfo->LITERAL_SUFFIX());
		idlcpy (raRecord[j +  5], pInfo->CREATE_PARAMS());
		idlcpy (raRecord[j +  6], pInfo->NULLABLE());
		idlcpy (raRecord[j +  7], pInfo->CASE_SENSITIVE());
		idlcpy (raRecord[j +  8], pInfo->SEARCHABLE());
		idlcpy (raRecord[j +  9], pInfo->UNSIGNED_ATTRIBUTE());
		idlcpy (raRecord[j + 10], pInfo->FIXED_PREC_SCALE());
		idlcpy (raRecord[j + 11], pInfo->AUTO_UNIQUE_VALUE());
		idlcpy (raRecord[j + 12], pInfo->LOCAL_TYPE_NAME());
		idlcpy (raRecord[j + 13], pInfo->MINIMUM_SCALE());
		idlcpy (raRecord[j + 14], pInfo->MAXIMUM_SCALE());
		idlcpy (raRecord[j + 15], pInfo->SQL_DATA_TYPE());
		idlcpy (raRecord[j + 16], pInfo->SQL_DATETIME_SUB());
		idlcpy (raRecord[j + 17], pInfo->NUM_PREC_RADIX());
		idlcpy (raRecord[j + 18], pInfo->INTERVAL_PRECISION());
	}
